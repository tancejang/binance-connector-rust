# Binance Rust VIP Loan Connector
...
This module provides the official Rust client for Binance's VIP Loan API, enabling developers to interact programmatically with Binance's VIP Loan trading platform. The library provides tools to over-collateralized loan service with competitive rates, terms, and services through the REST API:
...
Enable the `vip_loan` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["vip_loan"] }
```

In your code, import the `vip_loan` client:
...
- **Official Binance VIP Loan API docs:** [Binance API Documentation](https://developers.binance.com/docs/vip_loan)
...
The `vip_loan` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the VIP Loan module:

```bash
cargo test --features vip_loan
```
...
If you are upgrading from a legacy, single-crate connector that included VIP Loan support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `vip_loan` feature.
...
Please ensure that all tests pass with `--features vip_loan` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
