# Binance Rust Crypto Loan Connector
...
This module provides the official Rust client for Binance's Crypto Loan API, enabling developers to interact programmatically with Binance's Crypto Loan trading platform. The library provides tools to subscribe to fixed-term loans with fixed interest rates through the REST API:
...
Enable the `crypto_loan` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["crypto_loan"] }
```

In your code, import the `crypto_loan` client:
...
- **Official Binance Crypto Loan API docs:** [Binance API Documentation](https://developers.binance.com/docs/crypto_loan)
...
The `crypto_loan` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Crypto Loan module:

```bash
cargo test --features crypto_loan
```
...
If you are upgrading from a legacy, single-crate connector that included Crypto Loan support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `crypto_loan` feature.
...
Please ensure that all tests pass with `--features crypto_loan` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
