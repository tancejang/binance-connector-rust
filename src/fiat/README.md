...
This module provides the official Rust client for Binance's Fiat API, enabling developers to interact programmatically with Binance's Fiat trading platform. The library provides tools to get Binance Fiat Deposit/Withdraw history through the REST API:
...
Enable the `fiat` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["fiat"] }
```

In your code, import the `fiat` client:
...
- **Official Binance Fiat API docs:** [Binance API Documentation](https://developers.binance.com/docs/fiat)
...
The `fiat` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Fiat module:

```bash
cargo test --features fiat
```
...
If you are upgrading from a legacy, single-crate connector that included Fiat support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `fiat` feature.
...
Please ensure that all tests pass with `--features fiat` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
