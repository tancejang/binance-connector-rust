...
This module provides the official Rust client for Binance's Mining API, enabling developers to interact programmatically with Binance's Mining trading platform. The library provides tools to mine through the REST API:
...
Enable the `mining` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["mining"] }
```

In your code, import the `mining` client:
...
- **Official Binance Mining API docs:** [Binance API Documentation](https://developers.binance.com/docs/mining)
...
The `mining` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Mining module:

```bash
cargo test --features mining
```
...
If you are upgrading from a legacy, single-crate connector that included Mining support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `mining` feature.
...
Please ensure that all tests pass with `--features mining` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
