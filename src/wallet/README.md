...
This module provides the official Rust client for Binance's Wallet API, enabling developers to interact programmatically with Binance's Wallet trading platform. The library provides tools to interact with wallet related functions like dust convert/ query balance etc through the REST API:
...
Enable the `wallet` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["wallet"] }
```

In your code, import the `wallet` client:
...
- **Official Binance Wallet API docs:** [Binance API Documentation](https://developers.binance.com/docs/wallet)
...
The `wallet` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Wallet module:
...
If you are upgrading from a legacy, single-crate connector that included Wallet support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `wallet` feature.
...
Please ensure that all tests pass with `--features wallet` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
