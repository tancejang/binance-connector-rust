...
This module provides the official Rust client for Binance's Pay API, enabling developers to interact programmatically with Binance's Pay trading platform. The library provides tools to query transaction history through the REST API:
...
Enable the `pay` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["pay"] }
```

In your code, import the `pay` client:
...
- **Official Binance Pay API docs:** [Binance API Documentation](https://developers.binance.com/docs/pay)
...
The `pay` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Pay module:

```bash
cargo test --features pay
```
...
If you are upgrading from a legacy, single-crate connector that included Pay support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `pay` feature.
...
Please ensure that all tests pass with `--features pay` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
