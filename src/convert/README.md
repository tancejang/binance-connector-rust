# Binance Rust Convert Connector
...
This module provides the official Rust client for Binance's Convert API, enabling developers to interact programmatically with Binance's Convert trading platform. The library provides tools to buy or sell cryptocurrencies through the REST API:
...
Enable the `convert` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["convert"] }
```

In your code, import the `convert` client:
...
- **Official Binance Convert API docs:** [Binance API Documentation](https://developers.binance.com/docs/convert)
...
The `convert` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Convert module:

```bash
cargo test --features convert
```
...
If you are upgrading from a legacy, single-crate connector that included Convert support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `convert` feature.
...
Please ensure that all tests pass with `--features convert` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
