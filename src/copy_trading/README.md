# Binance Rust Copy Trading Connector
...
This module provides the official Rust client for Binance's Copy Trading API, enabling developers to interact programmatically with Binance's Copy Trading trading platform. The library provides tools to do automatic lead trading through the REST API:
...
Enable the `copy_trading` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["copy_trading"] }
```

In your code, import the `copy_trading` client:
...
- **Official Binance Copy Trading API docs:** [Binance API Documentation](https://developers.binance.com/docs/copy_trading)
...
The `copy_trading` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Copy Trading module:

```bash
cargo test --features copy_trading
```
...
If you are upgrading from a legacy, single-crate connector that included Copy Trading support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `copy_trading` feature.
...
Please ensure that all tests pass with `--features copy_trading` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
