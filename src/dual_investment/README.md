# Binance Rust Dual Investment Connector
...
Enable the `dual_investment` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["dual_investment"] }
```

In your code, import the `dual_investment` client:
...
- **Official Binance Dual Investment API docs:** [Binance API Documentation](https://developers.binance.com/docs/dual_investment)
...
The `dual_investment` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Dual Investment module:

```bash
cargo test --features dual_investment
```
...
If you are upgrading from a legacy, single-crate connector that included Dual Investment support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `dual_investment` feature.
...
Please ensure that all tests pass with `--features dual_investment` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
