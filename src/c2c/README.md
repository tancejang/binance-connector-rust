...
This module provides the official Rust client for Binance's C2C API, enabling developers to interact programmatically with Binance's C2C trading platform. The library provides tools to query Fiat transaction history through the REST API:
...
Enable the `c2c` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["c2c"] }
```

In your code, import the `c2c` client:
...
- **Official Binance C2C API docs:** [Binance API Documentation](https://developers.binance.com/docs/c2c)
...
The `c2c` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
If you are upgrading from a legacy, single-crate connector that included C2C support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `c2c` feature.
...
Please ensure that all tests pass with `--features c2c` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
