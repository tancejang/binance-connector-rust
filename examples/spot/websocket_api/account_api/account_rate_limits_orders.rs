...
use binance_sdk::spot::{SpotWsApi, websocket_api::AccountRateLimitsOrdersParams};
...
async fn main() -> Result<()> {
    ...
    let params = AccountRateLimitsOrdersParams::default();

    // Make the WS API call
    let response = connection
        .account_rate_limits_orders(params)
        .await
        .context("account_rate_limits_orders request failed")?;

    info!(?response.rate_limits, "account_rate_limits_orders rate limits");
    let data = response.data()?;
    info!(?data, "account_rate_limits_orders data");
