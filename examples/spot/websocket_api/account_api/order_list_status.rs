...
use binance_sdk::spot::{SpotWsApi, websocket_api::OrderListStatusParams};
...
async fn main() -> Result<()> {
    ...
    let params = OrderListStatusParams::default();

    // Make the WS API call
    let response = connection
        .order_list_status(params)
        .await
        .context("order_list_status request failed")?;

    info!(?response.rate_limits, "order_list_status rate limits");
    let data = response.data()?;
    info!(?data, "order_list_status data");
