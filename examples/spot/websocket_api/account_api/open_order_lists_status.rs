...
use binance_sdk::spot::{SpotWsApi, websocket_api::OpenOrderListsStatusParams};
...
async fn main() -> Result<()> {
    ...
    let params = OpenOrderListsStatusParams::default();

    // Make the WS API call
    let response = connection
        .open_order_lists_status(params)
        .await
        .context("open_order_lists_status request failed")?;

    info!(?response.rate_limits, "open_order_lists_status rate limits");
    let data = response.data()?;
    info!(?data, "open_order_lists_status data");
