...
use binance_sdk::spot::{SpotWsApi, websocket_api::OrderAmendmentsParams};
...
async fn main() -> Result<()> {
    ...
    let params = OrderAmendmentsParams::builder("BNBUSDT".to_string(), 1).build()?;

    // Make the WS API call
    let response = connection
        .order_amendments(params)
        .await
        .context("order_amendments request failed")?;

    info!(?response.rate_limits, "order_amendments rate limits");
    let data = response.data()?;
    info!(?data, "order_amendments data");
