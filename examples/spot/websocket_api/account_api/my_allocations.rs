...
use binance_sdk::spot::{SpotWsApi, websocket_api::MyAllocationsParams};
...
async fn main() -> Result<()> {
    ...
    let params = MyAllocationsParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .my_allocations(params)
        .await
        .context("my_allocations request failed")?;

    info!(?response.rate_limits, "my_allocations rate limits");
    let data = response.data()?;
    info!(?data, "my_allocations data");
