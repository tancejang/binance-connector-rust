...
use binance_sdk::spot::{SpotWsApi, websocket_api::AllOrderListsParams};
...
async fn main() -> Result<()> {
    ...
    let params = AllOrderListsParams::default();

    // Make the WS API call
    let response = connection
        .all_order_lists(params)
        .await
        .context("all_order_lists request failed")?;

    info!(?response.rate_limits, "all_order_lists rate limits");
    let data = response.data()?;
    info!(?data, "all_order_lists data");
