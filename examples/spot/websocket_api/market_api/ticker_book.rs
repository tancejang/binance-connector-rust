...
use binance_sdk::spot::{SpotWsApi, websocket_api::TickerBookParams};
...
async fn main() -> Result<()> {
    ...
    let params = TickerBookParams::default();

    // Make the WS API call
    let response = connection
        .ticker_book(params)
        .await
        .context("ticker_book request failed")?;

    info!(?response.rate_limits, "ticker_book rate limits");
    let data = response.data()?;
    info!(?data, "ticker_book data");
