...
use binance_sdk::spot::{SpotWsApi, websocket_api::TradesHistoricalParams};
...
async fn main() -> Result<()> {
    ...
    let params = TradesHistoricalParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .trades_historical(params)
        .await
        .context("trades_historical request failed")?;

    info!(?response.rate_limits, "trades_historical rate limits");
    let data = response.data()?;
    info!(?data, "trades_historical data");
