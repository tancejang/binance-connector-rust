...
use binance_sdk::spot::{SpotWsApi, websocket_api::TradesRecentParams};
...
async fn main() -> Result<()> {
    ...
    let params = TradesRecentParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .trades_recent(params)
        .await
        .context("trades_recent request failed")?;

    info!(?response.rate_limits, "trades_recent rate limits");
    let data = response.data()?;
    info!(?data, "trades_recent data");
