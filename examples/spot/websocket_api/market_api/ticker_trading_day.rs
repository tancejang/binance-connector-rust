...
use binance_sdk::spot::{SpotWsApi, websocket_api::TickerTradingDayParams};
...
async fn main() -> Result<()> {
    ...
    let params = TickerTradingDayParams::default();

    // Make the WS API call
    let response = connection
        .ticker_trading_day(params)
        .await
        .context("ticker_trading_day request failed")?;

    info!(?response.rate_limits, "ticker_trading_day rate limits");
    let data = response.data()?;
    info!(?data, "ticker_trading_day data");
