...
use binance_sdk::spot::{SpotWsApi, websocket_api::OrderListCancelParams};
...
async fn main() -> Result<()> {
    ...
    let params = OrderListCancelParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .order_list_cancel(params)
        .await
        .context("order_list_cancel request failed")?;

    info!(?response.rate_limits, "order_list_cancel rate limits");
    let data = response.data()?;
    info!(?data, "order_list_cancel data");
