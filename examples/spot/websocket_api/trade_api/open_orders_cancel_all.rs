...
use binance_sdk::spot::{SpotWsApi, websocket_api::OpenOrdersCancelAllParams};
...
async fn main() -> Result<()> {
    ...
    let params = OpenOrdersCancelAllParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .open_orders_cancel_all(params)
        .await
        .context("open_orders_cancel_all request failed")?;

    info!(?response.rate_limits, "open_orders_cancel_all rate limits");
    let data = response.data()?;
    info!(?data, "open_orders_cancel_all data");
