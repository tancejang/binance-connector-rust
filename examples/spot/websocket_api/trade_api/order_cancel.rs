...
use binance_sdk::spot::{SpotWsApi, websocket_api::OrderCancelParams};
...
async fn main() -> Result<()> {
    ...
    let params = OrderCancelParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .order_cancel(params)
        .await
        .context("order_cancel request failed")?;

    info!(?response.rate_limits, "order_cancel rate limits");
    let data = response.data()?;
    info!(?data, "order_cancel data");
