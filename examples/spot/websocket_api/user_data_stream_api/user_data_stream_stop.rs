...
use binance_sdk::spot::{SpotWsApi, websocket_api::UserDataStreamStopParams};
...
async fn main() -> Result<()> {
    ...
    let params = UserDataStreamStopParams::builder("listenKey".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .user_data_stream_stop(params)
        .await
        .context("user_data_stream_stop request failed")?;

    info!(?response.rate_limits, "user_data_stream_stop rate limits");
    let data = response.data()?;
    info!(?data, "user_data_stream_stop data");
