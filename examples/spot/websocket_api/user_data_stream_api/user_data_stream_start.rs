...
use binance_sdk::spot::{SpotWsApi, websocket_api::UserDataStreamStartParams};
...
async fn main() -> Result<()> {
    ...
    let params = UserDataStreamStartParams::default();

    // Make the WS API call
    let response = connection
        .user_data_stream_start(params)
        .await
        .context("user_data_stream_start request failed")?;

    info!(?response.rate_limits, "user_data_stream_start rate limits");
    let data = response.data()?;
    info!(?data, "user_data_stream_start data");
