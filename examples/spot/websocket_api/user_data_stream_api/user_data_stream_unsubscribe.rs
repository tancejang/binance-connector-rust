...
use binance_sdk::spot::{SpotWsApi, websocket_api::UserDataStreamUnsubscribeParams};
...
async fn main() -> Result<()> {
    ...
    let params = UserDataStreamUnsubscribeParams::default();

    // Make the WS API call
    let response = connection
        .user_data_stream_unsubscribe(params)
        .await
        .context("user_data_stream_unsubscribe request failed")?;

    info!(?response.rate_limits, "user_data_stream_unsubscribe rate limits");
    let data = response.data()?;
    info!(?data, "user_data_stream_unsubscribe data");
