...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("query_margin_available_inventory request failed")?;

    info!(?response.rate_limits, "query_margin_available_inventory rate limits");
    let data = response.data().await?;
    info!(?data, "query_margin_available_inventory data");
