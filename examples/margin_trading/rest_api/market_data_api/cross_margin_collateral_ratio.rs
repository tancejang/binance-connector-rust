...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("cross_margin_collateral_ratio request failed")?;

    info!(?response.rate_limits, "cross_margin_collateral_ratio rate limits");
    let data = response.data().await?;
    info!(?data, "cross_margin_collateral_ratio data");
