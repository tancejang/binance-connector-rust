...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("query_borrow_repay_records_in_margin_account request failed")?;

    info!(?response.rate_limits, "query_borrow_repay_records_in_margin_account rate limits");
    let data = response.data().await?;
    info!(?data, "query_borrow_repay_records_in_margin_account data");
