...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("margin_manual_liquidation request failed")?;

    info!(?response.rate_limits, "margin_manual_liquidation rate limits");
    let data = response.data().await?;
    info!(?data, "margin_manual_liquidation data");
