...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("get_boost_rewards_history request failed")?;

    info!(?response.rate_limits, "get_boost_rewards_history rate limits");
    let data = response.data().await?;
    info!(?data, "get_boost_rewards_history data");
