...
use binance_sdk::derivatives_trading_options::{
    DerivativesTradingOptionsRestApi, rest_api::ResetMarketMakerProtectionConfigParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("reset_market_maker_protection_config request failed")?;

    info!(?response.rate_limits, "reset_market_maker_protection_config rate limits");
    let data = response.data().await?;
    info!(?data, "reset_market_maker_protection_config data");
