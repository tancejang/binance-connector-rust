...
use binance_sdk::derivatives_trading_options::{
    DerivativesTradingOptionsRestApi, rest_api::ExtendBlockTradeOrderParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("extend_block_trade_order request failed")?;

    info!(?response.rate_limits, "extend_block_trade_order rate limits");
    let data = response.data().await?;
    info!(?data, "extend_block_trade_order data");
