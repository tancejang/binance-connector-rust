...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("set_locked_product_redeem_option request failed")?;

    info!(?response.rate_limits, "set_locked_product_redeem_option rate limits");
    let data = response.data().await?;
    info!(?data, "set_locked_product_redeem_option data");
