...
use binance_sdk::mining::{MiningRestApi, rest_api::RequestForDetailMinerListParams};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("request_for_detail_miner_list request failed")?;

    info!(?response.rate_limits, "request_for_detail_miner_list rate limits");
    let data = response.data().await?;
    info!(?data, "request_for_detail_miner_list data");
