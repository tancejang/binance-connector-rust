...
use binance_sdk::derivatives_trading_portfolio_margin::{
    DerivativesTradingPortfolioMarginRestApi, rest_api::ChangeCmInitialLeverageParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("change_cm_initial_leverage request failed")?;

    info!(?response.rate_limits, "change_cm_initial_leverage rate limits");
    let data = response.data().await?;
    info!(?data, "change_cm_initial_leverage data");
