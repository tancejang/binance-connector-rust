...
use binance_sdk::derivatives_trading_portfolio_margin::{
    DerivativesTradingPortfolioMarginRestApi, rest_api::CancelAllCmOpenConditionalOrdersParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("cancel_all_cm_open_conditional_orders request failed")?;

    info!(?response.rate_limits, "cancel_all_cm_open_conditional_orders rate limits");
    let data = response.data().await?;
    info!(?data, "cancel_all_cm_open_conditional_orders data");
