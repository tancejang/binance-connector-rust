...
use binance_sdk::derivatives_trading_portfolio_margin::{
    DerivativesTradingPortfolioMarginRestApi, rest_api::QueryCurrentCmOpenConditionalOrderParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("query_current_cm_open_conditional_order request failed")?;

    info!(?response.rate_limits, "query_current_cm_open_conditional_order rate limits");
    let data = response.data().await?;
    info!(?data, "query_current_cm_open_conditional_order data");
