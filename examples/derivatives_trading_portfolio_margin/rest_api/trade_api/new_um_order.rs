...
use binance_sdk::derivatives_trading_portfolio_margin::{
    ...
    rest_api::{NewUmOrderParams, NewUmOrderSideEnum, NewUmOrderTypeEnum},
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        .new_um_order(params)
        .await
        .context("new_um_order request failed")?;

    info!(?response.rate_limits, "new_um_order rate limits");
    let data = response.data().await?;
    info!(?data, "new_um_order data");
