...
use binance_sdk::crypto_loan::{CryptoLoanRestApi, rest_api::FlexibleLoanBorrowParams};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("flexible_loan_borrow request failed")?;

    info!(?response.rate_limits, "flexible_loan_borrow rate limits");
    let data = response.data().await?;
    info!(?data, "flexible_loan_borrow data");
