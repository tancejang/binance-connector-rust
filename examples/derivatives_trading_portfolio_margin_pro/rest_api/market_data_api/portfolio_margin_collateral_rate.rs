...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("portfolio_margin_collateral_rate request failed")?;

    info!(?response.rate_limits, "portfolio_margin_collateral_rate rate limits");
    let data = response.data().await?;
    info!(?data, "portfolio_margin_collateral_rate data");
