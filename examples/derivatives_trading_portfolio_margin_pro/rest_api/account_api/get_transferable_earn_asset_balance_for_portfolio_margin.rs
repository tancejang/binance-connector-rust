...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("get_transferable_earn_asset_balance_for_portfolio_margin request failed")?;

    info!(?response.rate_limits, "get_transferable_earn_asset_balance_for_portfolio_margin rate limits");
    let data = response.data().await?;
    info!(
        ?data,
        "get_transferable_earn_asset_balance_for_portfolio_margin data"
    );
