...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("query_managed_sub_account_futures_asset_details request failed")?;

    info!(?response.rate_limits, "query_managed_sub_account_futures_asset_details rate limits");
    let data = response.data().await?;
    info!(
        ?data,
        "query_managed_sub_account_futures_asset_details data"
    );
