...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("query_sub_account_assets_asset_management request failed")?;

    info!(?response.rate_limits, "query_sub_account_assets_asset_management rate limits");
    let data = response.data().await?;
    info!(?data, "query_sub_account_assets_asset_management data");
