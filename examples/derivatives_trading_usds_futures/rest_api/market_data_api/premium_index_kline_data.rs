...
use binance_sdk::derivatives_trading_usds_futures::{
    ...
    rest_api::{PremiumIndexKlineDataIntervalEnum, PremiumIndexKlineDataParams},
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("premium_index_kline_data request failed")?;

    info!(?response.rate_limits, "premium_index_kline_data rate limits");
    let data = response.data().await?;
    info!(?data, "premium_index_kline_data data");
