...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("toggle_bnb_burn_on_futures_trade request failed")?;

    info!(?response.rate_limits, "toggle_bnb_burn_on_futures_trade rate limits");
    let data = response.data().await?;
    info!(?data, "toggle_bnb_burn_on_futures_trade data");
