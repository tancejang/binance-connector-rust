...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesWsApi, websocket_api::PositionInformationParams,
};
...
async fn main() -> Result<()> {
    ...
    let params = PositionInformationParams::default();

    // Make the WS API call
    let response = connection
        .position_information(params)
        .await
        .context("position_information request failed")?;

    info!(?response.rate_limits, "position_information rate limits");
    let data = response.data()?;
    info!(?data, "position_information data");
