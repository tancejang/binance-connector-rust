...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesWsApi, websocket_api::PositionInformationV2Params,
};
...
async fn main() -> Result<()> {
    ...
    let params = PositionInformationV2Params::default();

    // Make the WS API call
    let response = connection
        .position_information_v2(params)
        .await
        .context("position_information_v2 request failed")?;

    info!(?response.rate_limits, "position_information_v2 rate limits");
    let data = response.data()?;
    info!(?data, "position_information_v2 data");
